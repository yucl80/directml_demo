...
import numpy
def to_numpy(tensor):
    return tensor.numpy() if tensor.requires_grad else tensor.numpy()    

#excute the model
beg = time.time()
for i in range(10):
    ort_inputs = {the_session.get_inputs()[0].name: input_ids.numpy(), the_session.get_inputs()[1].name: input_mask.numpy()}
